...
function ChatRoom() {
  ...
  return (
    <div className="flex h-full">
      ...
      {/* Chat List */}
      <div
        className={`w-1/3 border-r border-gray-300 bg-gray-100 ${showUserSearch || showCreateGroup ? "hidden md:block" : ""}`}
      >
        ...
        <div className="overflow-y-auto h-[calc(100%-110px)]">
          {chats.map((chat) => (
            <div
              key={chat.id}
              className={`p-3 border-b border-gray-200 cursor-pointer hover:bg-gray-200 ${activeChat?.id === chat.id ? "bg-gray-200" : ""}`}
      ...
      {/* Empty State */}
      {!activeChat && !showUserSearch && !showCreateGroup && (
        <div className="w-full md:w-2/3 flex flex-col items-center justify-center bg-white">
