...
const CourseDashboard = ({ expanded }) => {
      ...
      try {
        ...
        usersNameSnapshot.forEach((doc) => {
          if (
            ...
            !results.some((user) => user.id === doc.id)
          ) {
            results.push({
              id: doc.id,
              ...doc.data(),
  ...
  const sendMessage = async (e) => {
    ...
    if (!currentUser || !activeChat || !formValue.trim()) {
  ...
  const startChat = async (selectedUser) => {
    ...
    const existingChat = existingChats.docs.find((chatDoc) => {
      ...
      return (
        participants.includes(selectedUser.id) && participants.length === 2
      );
    });
            ...
            <div className="w-full h-full flex flex-col bg-white">
              <div className="p-4 border-b border-gray-300 flex items-center">
                ...
                <div>
                  ...
                  {activeChat.type !== "group" &&
                    activeChat.participantDetails[0] && (
                      <div className="flex items-center">
